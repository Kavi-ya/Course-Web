document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderModuleList(modules, hiddenIds, tabId) {
    ...
    const groups = processedModules.reduce((acc, mod) => {
      ...
      // Pattern 1: Old Format (2024 - Y1.S1 - Name)
      const oldMatch = mod.name.match(/^(\d{4}\s-\sY\d\.S\d)\s-\s(.+)$/);

      // Pattern 2: New Suffix Format (Name - Code [2024/JUL])
      const suffixMatch = mod.name.match(/^(.+?)\s-\s([A-Z0-9]+)\s+\[(\d{4}\/[A-Z]{3})\]$/);

      // Pattern 3: Simple Code Prefix (IT1234 - Name)
      const prefixMatch = mod.name.match(/^([A-Z]{2,4}\d{4})\s-\s(.+)$/);

      if (oldMatch) {
        groupName = oldMatch[1];
        moduleName = oldMatch[2];
      } else if (suffixMatch) {
        groupName = suffixMatch[3]; // 2024/JUL
        moduleName = `${suffixMatch[2]} - ${suffixMatch[1]}`; // Code - Name
      } else if (prefixMatch) {
        groupName = "General"; // Or maybe group by Code prefix like 'IT'
        moduleName = mod.name;
      }
  ...
  function setupTheme() {
    ...
    function applyTheme(theme) {
      if (theme === "light") {
        ...
        // Moon Icon
        themeToggle.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-moon"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path></svg>';
      }
    }
